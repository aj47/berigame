...
bpy.ops.export_scene.gltf(filepath=str(OUT/('starter-adventurer-v3-'+HAIR_ID+'.glb')),export_format='GLB',use_selection=True,export_animations=True,export_animation_mode='ACTIONS',export_force_sampling=True,export_skins=True,export_materials='EXPORT',export_yup=True)
mesh.calc_loop_triangles()
stats={'hair_id':HAIR_ID,'blender':bpy.app.version_string,'triangles':len(mesh.loop_triangles),'mesh_vertices':len(mesh.vertices),'materials':len(mesh.materials),'mesh_objects':1,'bones':len(arm.bones),'atlas':[64,64],'max_vertex_influences':max(len(w) for w in W),'clips':{n:{'seconds':v[0]/fps,'loop':v[1]} for n,v in specs.items()},'glb_bytes':(OUT/('starter-adventurer-v3-'+HAIR_ID+'.glb')).stat().st_size,'orientation':'Blender -Y forward, Z up; glTF +Z forward, Y up','status':'Asset build only; integration and device qualification tracked in docs/PLAYABLE_GRAPHICS_PROGRESS.md'}
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/('starter-adventurer-v3-'+HAIR_ID+'.blend')))
