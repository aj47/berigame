...
OUT = Path(__file__).resolve().parent / HAIR_ID
...
if HAIR_ID=='tousled':
    ...
    if HAIR_ID=='topknot':
        ...
        ellipsoid((0,.075,2.185),(.115,.12,.112),[16,17,18],'Head',8,4)
        tube((0,.055,2.13),(0,.055,2.165),[(0,.064,.062),(1,.062,.06)],17,'Head',8)
...
def pose(t=0,mode='Idle',strength=0):
    ...
    phase=2*pi*t; moving=mode in ['Run','Walk','RunGrab','RunGuard']; sway=sin(phase)
    strike=pulse(t,0,.32,.92) if mode=='Strike' else 0
    ...
    recoil=pulse(t,0,.22,1) if mode in ['Hit','Stagger','Grabbed'] else 0
    ...
    if mode in ['Hit','Stagger']:lean=-.22*recoil;rootz-=.06*recoil
    ...
    p['Chest'].rotation_mode='XYZ';p['Chest'].rotation_euler.z=(-.10*sway if moving else .12*strike+.10*counter)
    ...
    for s,lab in [(1,'L'),(-1,'R')]:
        ...
        if mode=='Strike':
            ...
            if lab=='R':
                target=target.lerp(Vector((-.21,-.52,1.43)),strike)
                direction=direction.lerp(Vector((0,-1,0)),strike)
                palmdir=(0,0,-1)
            pole=(s*.64,.0,1.16)
        ...
        if mode in ['Guard','Block','BlockCounter','RunGuard']:
            ...
            if mode=='BlockCounter' and lab=='R':
                ...
                palmdir=(0,0,-1)
        if mode in ['Hit','Stagger']:
            target.y+=.09*recoil;target.z+=.07*recoil
        if mode=='Grabbed':
        ...
        if mode=='Strike' and lab=='R':direction=direction.lerp(wrist-p['Forearm.'+lab].head,strike)
        ...
        p['Robe.'+lab].rotation_mode='XYZ';p['Robe.'+lab].rotation_euler.x=(.34*sign if moving else 0)
    if mode=='Turn':
        p['Root'].rotation_mode='XYZ';p['Root'].rotation_euler.y=pi/2*strength
    bpy.context.view_layer.update()

fps=30;scene.render.fps=fps
specs={'Idle':(60,True),'Run':(20,True),'RunGrab':(20,True),'RunGuard':(20,True),'Walk':(28,True),'Stop':(15,False),'Turn':(18,False),'Strike':(15,False),'Grab':(21,False),'GrabReady':(60,True),'Guard':(60,True),'Block':(12,False),'BlockCounter':(18,False),'Grabbed':(15,False),'Hit':(12,False),'Stagger':(15,False),'Defeat':(36,False)}
...
bpy.ops.export_scene.gltf(filepath=str(OUT/('starter-adventurer-v4-'+HAIR_ID+'.glb')),export_format='GLB',use_selection=True,export_animations=True,export_animation_mode='ACTIONS',export_force_sampling=True,export_skins=True,export_materials='EXPORT',export_yup=True)
...
for name,frame in ([] if '--skip-renders' in sys.argv else [('Run',5),('Strike',5),('Grab',7),('Grab',13),('GrabReady',1),('Guard',1),('BlockCounter',9),('Grabbed',4)]):
