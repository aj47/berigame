...
await page.goto('http://127.0.0.1:8767/viewer.html');await page.waitForFunction(()=>window.review?.ready,{timeout:30000});
const data=await page.evaluate(()=>({asset:review.asset,clips:review.clips,validation:review.validate()}));
for(const [clip,time] of [['Idle',0],['Run',.12],['Strike',.36],['Grab',.36],['GrabReady',.5],['Guard',.3],['Turn',.58],['Defeat',1.19]]){
...
data.note='Desktop headless Chrome, Three.js 0.149.0 (same version as game), isolated asset viewer. Not the game runtime or a smartphone benchmark.';
fs.writeFileSync(`${out}/web-validation.json`,JSON.stringify(data,null,2));console.log(JSON.stringify(data,null,2));
if(errors.length||data.validation.some(r=>!r.finite||r.minY<-.02||r.maxExtent>3)||data.asset.triangles>2500||data.asset.materials!==1||data.clips.length!==12)throw new Error('Asset validation failed');
await browser.close();
